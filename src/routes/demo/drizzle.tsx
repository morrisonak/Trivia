...
// API endpoints will be defined in separate API route files
const todosApi = {
  getTodos: async () => {
    const response = await fetch('/demo/api/d1-todos')
...
function DemoDrizzle() {
  const [todos, setTodos] = useState<Array<{ id: number; title: string; createdAt: string | null }>>([])
      ...
      >
        ...
        >
          <h3 className="text-lg font-semibold mb-2 text-indigo-200">
            Powered by Drizzle ORM + Cloudflare D1
          </h3>
          <p className="text-sm text-indigo-300/80 mb-4">
            Type-safe ORM with Cloudflare's serverless SQLite database
          </p>
          <div className="space-y-2 text-sm">
            <p className="text-indigo-200 font-medium">Key Features:</p>
            <ul className="list-disc list-inside space-y-2 text-indigo-300/80">
              <li>Cloudflare D1 (SQLite) for serverless data storage</li>
              <li>Drizzle ORM for type-safe database queries</li>
              <li>Zero-latency at the edge with Cloudflare Workers</li>
              <li>Automatic schema migrations with drizzle-kit</li>
            </ul>
          </div>
        </div>
      </div>
